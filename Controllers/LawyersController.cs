...
public class LawyersController : ControllerBase
{
        ...
        [FromQuery] int? maxExperience)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q))
        {
            ...
            query = query.Where(l => l.FullName.ToLower().Contains(term) || l.Specialization.ToLower().Contains(term));
        }
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        var activeClients = await _db.Cases.CountAsync(c => c.LawyerId == l.Id && c.Status == "active");
        return Ok(new
        {
            id = l.Id,
            name = l.FullName,
